...
int main() {
    int board[SIZE][SIZE] = {0};  // 初始棋盤設為0
    int turns = 0; // 用於跟踪回合數
    int row, column;
    
    while (scanf("%d,%d", &row, &column) != EOF) {
        turns++;
        int player = (turns % 2 == 1) ? 1 : 2; // 奇數回合為1(黑棋)，偶數回合為2(白棋)
        board[row+4][column+4] = player; // 將輸入的行和列轉換成棋盤位置

        // 計算行和列的棋子數量
        
        int count_row = 0, count_col = 0;
        for (int i = 0; i < SIZE; i++) {
            if (board[row+4][i] == 1 || board[row+4][i] == 2) {
                count_row++;
            }
            if (board[i][column+4] == 1 || board[i][column+4] == 2) {
                count_col++;
            }
        }

        printf("%d:%d %d:%d\n", row, count_row, column, count_col);
